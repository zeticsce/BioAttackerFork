...
async def improve(message: types.Message):
    ...
    ''' ПРОКАЧКА БЕЗОПАСНОСТИ '''

    if message.text.lower().startswith("+бп ") or message.text.lower().startswith("+безопасность "):
        if lab.has_lab:  
            msg = message.text.lower().split(" ")
            text = "🔬 _Меню прокачки уровней_\n\n"
            current_security = lab.security
            ...
            if len(msg) == 2:
                if msg[1].isdigit():
                    ...
                    total_cost = calculate.bp(current_security, (current_security+level))
                    total_cost = str('{0:,}'.format(total_cost).replace(',', ' '))

                    text += f"Ваш текущий уровень безопасности: `{current_security}` 🕵️‍♂️\n"
                    text += f"Улучшение на _+{level}_ будет стоить: "
                    text += f"`{total_cost}` 🧬\n\n"
                    text += f"*Чтобы подвердить улучшение напишите:* `++бп {level}`"
            ...
            if len(msg) == 2:
                if msg[1].isdigit():
                    ...
                    if current_balance < total_cost:
                        ...
                        lab.save()
                        total_cost = str('{0:,}'.format(total_cost).replace(',', ' '))
                        text += f"Вы успешно добавили `{level}` безопасности!\n"
                        text += f"С баланса снято `{total_cost}` био-ресурсов 🧬"
                        await message.reply(text=text, parse_mode="Markdown")
