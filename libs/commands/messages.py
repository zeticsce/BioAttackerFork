...
🥽 Предотвращено: {prevented_issue}/{all_issue} (<code>{issues_percent}%</code>){fever}""",
            ...
            "no corp": "",
            "next patogen sec": "\n⏱ До новой инфекции: <code>{next_patogen_time}</code> сек.",
            "next patogen min": "\n⏱ До новой инфекции: <code>{next_patogen_time}</code> мин.",
            "full patogens": "",
...
def attackText(theme, new, first_name, second_name, first_id, second_id, patogen_name, atts, profit, mortality):
    ...
    if theme == "azeri":
        ...
        lol = "👨‍🔬 Объект ещё не подвергался заражению вашим мутагеном"
